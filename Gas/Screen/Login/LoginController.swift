...
//            self.postGetToken()
            ...
            postGetToken.postGetToken_Block(info: showUserName, pass: showPass, companyCode: showcompanyCode  ) { _ in
...
//            }
    ...
    func callApi_Async_Await() async {
        ...
        var detailGetToken = await getToken.getToken_Async_Await(userName: txtUserName.text ?? "", pass: txtPass.text ?? "", companyCode: txtcompanyCode.text ?? "")
        print(detailGetToken)
        
        let getMe = GetMe_Async_Await()
        let detailGetMe = await getMe.getMe_Async_Await()
        print(detailGetMe)
